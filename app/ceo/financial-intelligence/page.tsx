...
const generateChartData = (entries: any[]): ChartData[] => {
    ...
    for (let i = 29; i >= 0; i--) {
        ...
        const uloomx = dayEntries.reduce(
            (sum, entry) => sum + (parseFloat(entry.uloomx_income) || 0),
            0,
        );
        const usthad = dayEntries.reduce(
            (sum, entry) => sum + (parseFloat(entry.usthad_income) || 0),
            0,
        );
        const total = uloomx + usthad;
        const expenses = dayEntries.reduce(
            (sum, entry) => sum + (parseFloat(entry.total_expenses) || 0),
            0,
        );
    ...
    return (
        <div className="bg-white rounded-[20px] md:rounded-[24px] border border-gray-100 shadow-sm p-4 md:p-6 hover:shadow-lg transition-all duration-300 group">
            <div className="flex items-start justify-between mb-3 md:mb-4">
                <div
                    className={`rounded-xl md:rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-105 overflow-hidden flex-shrink-0 ${
                        imageUrl?.includes("uloomx")
                ...
                {trend && trendValue && (
                    <div
                        className={`flex items-center gap-0.5 md:gap-1 text-xs md:text-sm font-bold px-2 md:px-3 py-1 md:py-1.5 rounded-full flex-shrink-0 ${
                            trend === "up"
            ...
            <p className="text-xs md:text-sm text-[#64748b] font-medium mb-1 md:mb-2 truncate">
                {title}
            </p>
            <p className="text-xl md:text-3xl font-black text-[#1e293b] truncate">
                {value}
            </p>
        </div>
    );
...
const StackedAreaChart = ({ data }: { data: ChartData[] }) => {
            ...
            <div className="relative w-full h-[280px] flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-400 text-sm mt-2">
                        Submit financial entries to see trends
                    </p>
                </div>
            </div>
        );
    ...
    const { usthadAreaPath, uloomxAreaPath, uloomxPoints } =
        createStackedPaths();
    ...
    const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
        ...
        const index = Math.round(
            ((x - scaledPaddingLeft) / chartWidth) * (data.length - 1),
        );
            ...
            >
                ...
                {data
                    ...
                    .map((point, index) => {
                        const x =
                            padding.left +
                            (data.indexOf(point) / (data.length - 1)) *
                                (width - padding.left - padding.right);
...
const DonutChart = ({
    ...
    const uloomxOffset =
        circumference - (uloomxPercentage / 100) * circumference;
    const usthadOffset =
        circumference - (usthadPercentage / 100) * circumference;
        ...
        >
            <div className="relative" style={{ width: size, height: size }}>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <p className="text-[10px] md:text-xs text-[#64748b] font-medium">
                        Total Revenue
                    </p>
                    <p className="text-sm md:text-lg font-black text-[#1e293b]">
...
export default function CEOFinancialIntelligence() {
    ...
    const downloadMonthlyFinanceReport = async () => {
        ...
        try {
            ...
            const drawFooterConfidential = (d: jsPDF) => {
                ...
                const footerMsg =
                    "CONFIDENTIAL - USTHAD ACADEMY COMMAND CENTER OS OFFICIAL FINANCIAL AUDIT REPORT.";
            ...
            const drawHeaderBanner = (d: jsPDF, pageNum: number) => {
                ...
                d.setFontSize(8);
                d.text(
                    "COMMAND CENTER OS • MONTHLY FINANCIAL AUDIT REPORT",
                    38,
                    22,
                );
            ...
            doc.text(
                "COMMAND CENTER OS • FINANCIAL INTELLIGENCE SYSTEM",
                105,
                78,
                { align: "center" },
            );
            ...
            doc.text("MONTHLY FINANCIAL AUDIT BRIEFING", 105, 96, {
                align: "center",
            });
            ...
            doc.text(`AUDIT REPORT PERIOD: ${reportMonthName}`, 105, 103, {
                align: "center",
            });
            ...
            const uloomxTotal =
                entries?.reduce(
                    (sum, e) => sum + (parseFloat(e.uloomx_income) || 0),
                    0,
                ) || 0;
            const usthadTotal =
                entries?.reduce(
                    (sum, e) => sum + (parseFloat(e.usthad_income) || 0),
                    0,
                ) || 0;
            const totalExpense =
                entries?.reduce(
            ...
            const targetsList = [
                {
                    name: "Usthad Academy Revenue Target",
                    val: targets.usthadTarget,
                    actual: usthadTotal,
                    color: "#31267D",
                },
                {
                    name: "UloomX Revenue Target",
                    val: targets.uloomxTarget,
                    actual: uloomxTotal,
                    color: "#F14D24",
                },
                {
                    name: "Operational Expense limit",
                    val: targets.expenseTarget,
                    actual: totalExpense,
                    color: "#EF4444",
                },
            ];
            ...
            targetsList.forEach((t) => {
                ...
                doc.text(`${percentage.toFixed(1)}% Achieved`, 186, yPos + 9, {
                    align: "right",
                });
            ...
            if (!entries || entries.length === 0) {
                ...
                entries.forEach((e: any, index: number) => {
                    const entryDate = new Date(e.entry_date).toLocaleDateString(
                        "en-IN",
                        {
                            day: "numeric",
                            month: "short",
            ...
            doc.text(
                "Executive Financial Authority Signature",
                15,
                tableYPos + 9,
            );
            ...
            // Save PDF
            doc.save(
                `Usthad_Academy_Financial_Report_${reportMonthName.replace(/\s+/g, "_")}.pdf`,
            );
        ...
        try {
            ...
            if (data && !error) {
                ...
                localStorage.setItem(
                    "ua_financial_targets",
                    JSON.stringify({ ...dbTargets, month: monthStr }),
                );
                return;
            }
        } catch (err) {
    ...
    const animatedUloomxIncome = useCounterAnimation(
        dailyMetrics.uloomxIncome,
        2000,
    );
    const animatedUsthadIncome = useCounterAnimation(
        dailyMetrics.usthadIncome,
        1800,
    );
    const animatedCombinedExpense = useCounterAnimation(
        dailyMetrics.combinedExpense,
        2200,
    );
    const animatedNetProfit = useCounterAnimation(dailyMetrics.netProfit, 2500);

    // Animated values for monthly metrics
    const animatedMonthlyUloomx = useCounterAnimation(
        monthlyMetrics.uloomxTotal,
        2800,
    );
    const animatedMonthlyUsthad = useCounterAnimation(
        monthlyMetrics.usthadTotal,
        3000,
    );
    const animatedMonthlyExpense = useCounterAnimation(
        monthlyMetrics.totalExpense,
        2600,
    );
    ...
    const getTrendProps = (
        ...
        return {
            trend: isExpense
                ? percentage <= 0
                    ? ("up" as const)
                    : ("down" as const) // For expense: decrease is positive signal (green, up), increase is negative (red, down)
                : percentage >= 0
                  ? ("up" as const)
                  : ("down" as const),
            trendValue,
    ...
    useEffect(() => {
        ...
        const todayUloomx = todayEntries.reduce(
            (sum, entry) => sum + (parseFloat(entry.uloomx_income) || 0),
            0,
        );
        const todayUsthad = todayEntries.reduce(
            (sum, entry) => sum + (parseFloat(entry.usthad_income) || 0),
            0,
        );
        ...
        const yesterdayEntries = financialEntries.filter(
            (entry) => entry.entry_date === yesterdayStr,
        );

        const yesterdayUloomx = yesterdayEntries.reduce(
            (sum, entry) => sum + (parseFloat(entry.uloomx_income) || 0),
            0,
        );
        const yesterdayUsthad = yesterdayEntries.reduce(
            (sum, entry) => sum + (parseFloat(entry.usthad_income) || 0),
            0,
        );
        const yesterdayExpenses = yesterdayEntries.reduce(
            (sum, entry) => sum + (parseFloat(entry.total_expenses) || 0),
            0,
        );
    ...
    */
    ...
    const balanceTarget =
        targets.uloomxTarget + targets.usthadTarget - targets.expenseTarget;
    const expenseRatio =
        (monthlyMetrics.totalExpense /
            Math.max(
                1,
                monthlyMetrics.uloomxTotal + monthlyMetrics.usthadTotal,
            )) *
        100;
    const targetAchievement =
        (monthlyMetrics.balance / Math.max(1, balanceTarget)) * 100;
                ...
                <div className="max-w-7xl mx-auto">
                        ...
                        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 md:gap-6">
                            ...
                            <div className="flex items-center gap-3 md:gap-5 w-full lg:w-auto">
                                ...
                                <div
                                    ...
                                    style={{
                                        background:
                                            "linear-gradient(135deg, #ff4d00 0%, #dc2626 100%)",
                                        boxShadow:
                                            "0 12px 40px rgba(255, 77, 0, 0.35)",
                                    }}
                                >
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            <div className="mb-8">
                                <h2 className="text-lg md:text-xl font-semibold text-[#1e293b] mb-4">
                                    Real-Time Daily Transmission
                                </h2>
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                                    <MetricCard
                                        title="Today's UloomX Income"
                                        value={formatCurrency(
                                            animatedUloomxIncome.currentValue,
                                        )}
                                        imageUrl="/images/uloomx.png"
                                        trend={
                                            getTrendProps(
                                                dailyMetrics.uloomxIncome,
                                                yesterdayMetrics.uloomxIncome,
                                            ).trend
                                        }
                                        trendValue={
                                            getTrendProps(
                                    ...
                                    <MetricCard
                                        ...
                                        value={formatCurrency(
                                            animatedUsthadIncome.currentValue,
                                        )}
                                        imageUrl="/images/usthadacademylogo2.svg"
                                        trend={
                                            getTrendProps(
                                                dailyMetrics.usthadIncome,
                                                yesterdayMetrics.usthadIncome,
                                            ).trend
                                        }
                                        trendValue={
                                            getTrendProps(
                                    ...
                                    <MetricCard
                                        ...
                                        value={formatCurrency(
                                            animatedCombinedExpense.currentValue,
                                        )}
                                        icon={Wallet}
                                        trend={
                                            getTrendProps(
                                                dailyMetrics.combinedExpense,
                                                yesterdayMetrics.combinedExpense,
                                                true,
                                            ).trend
                                        }
                                        trendValue={
                                    ...
                                    <MetricCard
                                        ...
                                        value={formatCurrency(
                                            animatedNetProfit.currentValue,
                                        )}
                                        icon={TrendingUp}
                                        trend={
                                            getTrendProps(
                                                dailyMetrics.netProfit,
                                ...
                                <div className="bg-white rounded-[20px] md:rounded-[24px] border border-gray-100 shadow-lg p-4 md:p-6 lg:p-10">
                                    <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
                                        ...
                                        <div className="flex-1">
                                            <h3 className="text-lg md:text-xl font-bold text-[#1e293b] mb-4 md:mb-8">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                                                ...
                                                <div className="p-4 md:p-6 rounded-[16px] md:rounded-[20px] bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 hover:shadow-md transition-all flex flex-col justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-xs md:text-sm text-[#64748b] mb-1">
                                                            Monthly Usthad Total
                                                        </p>
                                                        <p className="text-xl md:text-2xl font-black text-[#1e293b]">
                                                            {formatCurrency(
                                                                animatedMonthlyUsthad.currentValue,
                                                            )}
                                                        </p>
                                                    </div>
                                                        ...
                                                        <div className="w-full bg-slate-200 dark:bg-zinc-800 rounded-full h-1.5 overflow-hidden">
                                                            <div
                                                                ...
                                                                style={{
                                                                    ...
                                                                    background:
                                                                        "linear-gradient(90deg, #ffb088 0%, #ff8c52 100%)",
                                                                }}
                                                            />
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="p-4 md:p-6 rounded-[16px] md:rounded-[20px] bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 hover:shadow-md transition-all flex flex-col justify-between">
                                                    <div>
                                                        <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-white border border-slate-100 shadow-sm flex items-center justify-center mb-2 md:mb-3 overflow-hidden p-0.5">
                                                            {/* eslint-disable-next-line @next/next/no-img-element */}
                                                            <img
                                                                src="/images/uloomx.png"
                                                                alt="UloomX"
                                                                className="w-full h-full object-contain scale-110"
                                                            />
                                                        </div>
                                                        <p className="text-xs md:text-sm text-[#64748b] mb-1">
                                                            Monthly UloomX Total
                                                        </p>
                                                        <p className="text-xl md:text-2xl font-black text-[#1e293b]">
                                                            {formatCurrency(
                                                                animatedMonthlyUloomx.currentValue,
                                                            )}
                                                        </p>
                                                    </div>
                                                    <div className="mt-4 pt-3 border-t border-gray-200/50 flex flex-col gap-1.5 w-full">
                                                        ...
                                                        <div className="w-full bg-slate-200 dark:bg-zinc-800 rounded-full h-1.5 overflow-hidden">
                                                            <div
                                                                ...
                                                                style={{
                                                                    ...
                                                                    background:
                                                                        "linear-gradient(90deg, #ff4d00 0%, #ff6b35 100%)",
                                                                }}
                                                            />
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="p-4 md:p-6 rounded-[16px] md:rounded-[20px] bg-gradient-to-br from-red-50 to-red-100 border border-red-200 hover:shadow-md transition-all flex flex-col justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-xl md:text-2xl font-black text-[#1e293b]">
                                                            {formatCurrency(
                                                                animatedMonthlyExpense.currentValue,
                                                            )}
                                                        </p>
                                                    </div>
                                        ...
                                        <div className="lg:w-[420px]">
                                            <div
                                                className="h-full p-4 md:p-8 rounded-[16px] md:rounded-[20px] border-2 flex flex-col justify-between"
                                                style={{
                                                    borderColor: "#ff4d00",
                                                    background:
                                                        "linear-gradient(135deg, rgba(255,77,0,0.08) 0%, rgba(255,107,53,0.03) 100%)",
                                                }}
                                            >
                                                <div>
                                                    <p className="text-xs md:text-sm text-[#64748b] font-medium mb-1 md:mb-2">
                            ...
                            <div>
                                <h2 className="text-lg md:text-xl font-semibold text-[#1e293b] mb-4">
                                    Growth Analytics & Insight Engine
                                </h2>
                                <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 md:gap-6">
                                    ...
                                    <div className="lg:col-span-3 bg-white rounded-[20px] md:rounded-[24px] border border-gray-100 shadow-lg p-4 md:p-8">
                                        <h3 className="text-base md:text-lg font-bold text-[#1e293b] mb-4 md:mb-6">
                                    ...
                                    <div className="lg:col-span-2 space-y-4 md:space-y-6">
                                        ...
                                        <div className="space-y-4">
                                            {/* Expense Alert */}
                                            <div
                                                className={`p-4 md:p-5 rounded-[16px] md:rounded-[20px] border transition-all ${
                                                    monthlyMetrics.totalExpense >
                                                    targets.expenseTarget
                                                ...
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-xs text-[#64748b] mt-1">
                                                            ...
                                                            {targetAchievement <
                                                                100 &&
                                                                ` - ₹${formatCurrency(balanceTarget - monthlyMetrics.balance).replace("₹", "")} more needed`}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    {financialEntries.length === 0 ? (
                                        <div className="p-12 text-center">
                                            ...
                                            <p className="text-gray-400 text-sm mt-1">
                                                Financial logs will appear once
                                                submitted by accounts personnel.
                                            </p>
                                        </div>
                                    ) : (
                                            ...
                                            <table className="w-full text-left border-collapse">
                                                <thead>
                                                    <tr className="bg-slate-50/50 border-b border-gray-100">
                                                        <th className="p-4 md:p-5 text-xs font-bold uppercase tracking-wider text-[#64748b]">
                                                            Date
                                                        </th>
                                                        <th className="p-4 md:p-5 text-xs font-bold uppercase tracking-wider text-[#64748b]">
                                                            UloomX Income
                                                        ...
                                                        (entry: any) => {
                                                            ...
                                                            return (
                                                                ...
                                                                >
                                                                    ...
                                                                    <td
                                                                        className={`p-4 md:p-5 text-xs md:text-sm font-black whitespace-nowrap ${netVal >= 0 ? "text-green-600" : "text-red-600"}`}
                                                                    >
                                                                        {formatCurrency(
                                                                            netVal,
                                                                        )}
...
function SetAcademyTargetsModal({
    ...
    const [usthadVal, setUsthadVal] = useState(
        currentTargets.usthadTarget.toString(),
    );
    const [uloomxVal, setUloomxVal] = useState(
        currentTargets.uloomxTarget.toString(),
    );
    const [expenseVal, setExpenseVal] = useState(
        currentTargets.expenseTarget.toString(),
    );
                ...
                <form onSubmit={handleFormSubmit} className="space-y-5">
                    ...
                    <div className="space-y-2.5 p-3.5 bg-slate-50/50 dark:bg-zinc-800/30 rounded-2xl border border-slate-100 dark:border-zinc-800/50">
                        <div className="flex justify-between items-center select-none">
                            <Label
                                htmlFor="target-usthad"
                                className="text-xs font-black uppercase text-slate-500 dark:text-zinc-400"
                            >
