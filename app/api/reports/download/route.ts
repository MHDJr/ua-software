...
function buildPDF(type: string, year: number, month: number, data: any[]): Promise<Buffer> {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Iterate data rows
            if (data.length === 0) {
                // If empty, draw a nice blank row
                ...
                data.forEach((item, index) => {
                    const rowHeight = 22;
                    // Pagination Check: if too low on the page, create a new page and draw header details
                    if (yPos + rowHeight > 730) {
                        doc.addPage();
                        // Draw header details at the top of the new page
