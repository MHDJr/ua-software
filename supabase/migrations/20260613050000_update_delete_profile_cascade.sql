-- =====================================================
-- COMPREHENSIVE CASCADE DELETION FOR PROFILES (V4)
-- =====================================================
-- This updates delete_profile_cascade to also delete the user from auth.users,
-- and clean up pending signup/add_staff requests.
...
BEGIN
    ...
    IF v_email IS NULL THEN
        SELECT email INTO v_email FROM auth.users WHERE id = profile_uuid;
    END IF;

    -- Security Check: Ensure caller is CEO
    IF NOT EXISTS (
        SELECT 1 FROM profiles 
        WHERE id = auth.uid() AND (LOWER(role) = 'ceo')
    ) THEN
        RAISE EXCEPTION 'Access Denied: Only the CEO can delete profiles.';
    END IF;
    ...
    -- 25a. staff_push_tokens
    BEGIN
        DELETE FROM staff_push_tokens WHERE user_id = profile_uuid;
    EXCEPTION WHEN OTHERS THEN NULL;
    END;

    -- 25b. signup_requests and pending requests cleanup
    BEGIN
        IF v_email IS NOT NULL THEN
            DELETE FROM signup_requests WHERE LOWER(email) = LOWER(v_email);
            DELETE FROM requests WHERE type = 'add_staff' AND LOWER(metadata->>'email') = LOWER(v_email);
        END IF;
        IF v_username IS NOT NULL THEN
            DELETE FROM signup_requests WHERE LOWER(username) = LOWER(v_username);
        END IF;
    EXCEPTION WHEN OTHERS THEN NULL;
    END;

    -- 26. Delete the profile from profiles table
    BEGIN
        DELETE FROM profiles WHERE id = profile_uuid;
    EXCEPTION WHEN OTHERS THEN NULL;
    END;

    -- 27. Delete from auth.users (completely deletes the authenticated user)
    BEGIN
        DELETE FROM auth.users WHERE id = profile_uuid;
    EXCEPTION WHEN OTHERS THEN NULL;
    END;
